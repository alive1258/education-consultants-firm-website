...
const Testimonial = () => {
  ...
  const hardcodedStudents: StudentProfile[] = [
    {
      id: 1,
      name1: "Alex Johnson",
      name2: "Sarah Chen",
  ...
  const getCountryFromName = (name: string) => {
    const nameToCountry: Record<string, string> = {
      Alex: "United States",
      Sarah: "Singapore",
      Mohammed: "Bangladesh",
  ...
  const getProgramFromUniversity = (university: string) => {
    const universityPrograms: Record<string, string> = {
  ...
  const getGraduationYear = (studentNum: number) => {
    const years = ["2023", "2022", "2024"];
    return years[(studentNum - 1) % 3];
  };

  // Function to generate rating
  const getRating = (studentNum: number) => {
    const ratings = [5, 4, 5];
    return ratings[(studentNum - 1) % 3];
  };

  const StudentCard = (studentNum: number): StudentCardData => {
    const studentDataMap: Record<number, StudentDataItem> = {
      1: {
    ...
    const studentData = studentDataMap[studentNum];
      ...
      <div className="container mx-auto px-4">
        ...
        <div className="grid md:grid-cols-3 gap-8 mb-12">
          {[1, 2, 3].map((studentNum) => {
            ...
            return (
              <div key={studentNum} className="group relative">
                {/* Card */}
                <div className="bg-white rounded-2xl shadow-lg border border-gray-200 p-8 h-full hover:shadow-2xl transition-all duration-500 hover:-translate-y-2 min-h-125 flex flex-col">
                  {/* Quote Icon */}
                ...
                <div className="absolute -top-3 -right-3 bg-linear-to-r from-blue-600 to-cyan-500 text-white px-4 py-2 rounded-full font-bold text-sm shadow-lg">
                  100% Scholarship
                </div>
              </div>
            );
